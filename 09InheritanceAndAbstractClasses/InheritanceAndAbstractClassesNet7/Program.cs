...
namespace InheritanceAndAbstractClasses
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            Polygon p = new Polygon(new Point(2, 2), Color.FromArgb(0, 255, 0), 5);
            ...
            {
                using Bitmap bmp1 = new(600, 400);
                using Graphics gfx1 = Graphics.FromImage(bmp1);
                sh.Draw(gfx1);
                Console.WriteLine($"{sh.GetType()}'s area is {sh.Area} {((Polygon)sh).Sparkle()}");
            }
            using Bitmap bmp2 = new(600, 400);
            using Graphics gfx2 = Graphics.FromImage(bmp2);
            polygon.Draw(gfx2);
